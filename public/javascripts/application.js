...
quickhitch.controller('rideController', function ($scope, Data) {
	...
	*/
	...
	$scope.findRide = function() {
		...
		if(!_.isEmpty(Data.matches)) { //if there are potential matches
			//remove current user from the match list.
			Data.matches = _.reject(Data.matches, function(val) {return val.name === $scope.ride.name})
